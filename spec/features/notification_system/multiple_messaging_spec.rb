...
RSpec.feature 'Messaging otification on navigation panel', '
  ...
  scenario 'Registered sees a notification when messaged' do
    ...
    in_browser(:one) do
      ...
      as_user(user) do
        ...
        messaging_notification_counter_is_at(2)
      end
    end
  end
  ...
  let!(:pepito){User.create(email: 'bkooooo@bar.com', password: 'password', first_name: 'Jacke', last_name: 'Bauerss')}
  let!(:friending){
    ...
    user.friend_request(pepito)
    pepito.accept_request(user)
    conv = Conversation.create!(sender_id: user.id, recipient_id: pepito.id)
    ConversationNotification.create!(user_id: user.id, conversation_id: conv.id, status: false)
    ConversationNotification.create!(user_id: pepito.id, conversation_id: conv.id, status: false)
  }
