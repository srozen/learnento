...
RSpec.describe 'Fetch a single conversation notification of a user', type: :request do
  ...
  context 'Request a single conversation_notification with a valid JWT for user with two conversations' do
    ...
    before(:each) do
      ...
      conversation = user.conversation_with(charlie)
  ...
  context 'Request a conversation_notification after receiving a message' do
    before(:each) do
      headers = request_headers
      headers[:'HTTP_AUTHORIZATION'] = "Bearer #{jwt}"
      put "/api/friend_requests/#{otheruser.id}", request_body(otheruser.id), headers
    ...
    it 'returns a TRUE response status, a new message appeared' do
      expect(response_body).to include('status')
