...
RSpec.describe 'Check single friend statuses through the api', type: :request do
  ...
  context 'Request an existing friended user with a valid JWT' do
    it 'returns true' do
      headers = request_headers
      headers[:'HTTP_AUTHORIZATION'] = "Bearer #{jwt}"
      get "/api/friends/#{puser.id}", '', headers
  ...
  context 'Request an non-existing requested_friend with a valid JWT' do
    it 'returns false' do
      headers = request_headers
      headers[:'HTTP_AUTHORIZATION'] = "Bearer #{jwt}"
      get "/api/friends/#{otheruser.id}", '', headers
      expect(response.status).to eq 200
      expect(response_body).to include('status')
